...
//Now suppose we need to reduce demand 20%

range rangeProducts=0..card(products)-1;

execute
{
for(var indexProd in rangeProducts)
{
    var p=Opl.item(products,indexProd);
    p.demand=0.8*p.demand;
}
writeln(products);
}
